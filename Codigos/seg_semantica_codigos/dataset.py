        ...
        for it in data_list:
            if it[0].isdigit():
                ...
                if new_data <= self.new_data_size:
                    item = (os.path.join(root, self.dataset, 'images', it), os.path.join(root, self.dataset, 'masks', it))
                    items.append(item)
            else:
                item = (os.path.join(root, self.dataset, 'images', it), os.path.join(root, self.dataset, 'masks', it))
                items.append(item)
...
# Class that reads a sequence of image paths from a text file and creates a data.Dataset with them.
class ListDataset_forward(data.Dataset):
    ...
    def make_dataset(self):
        ...
        for it in data_list:
            ...
            #         item = (os.path.join(root, self.dataset, 'images', it), os.path.join(root, self.dataset, 'masks', it))
            #         items.append(item)
            #         print(os.path.join(root, self.dataset, 'images', it))
            # else:
            item = (os.path.join(root, self.dataset, 'images', it), os.path.join(root, self.dataset, 'masks', it))
