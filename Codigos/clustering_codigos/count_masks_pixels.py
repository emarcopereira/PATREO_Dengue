...
#all_files = os.listdir('/mnt/DADOS_PARIS_1/matheusp/dengue/chessmix/forward_outputs/fcn-resnet101/probs_campinas_inteira')
...
def count_classes (root):
    ...
    for classe in [1,2,3]:
        ...
        for file in files:
            X[index][9] = classe
            X[index][10] = int(file.split('.')[0])
            prds = np.load(root+"classe"+str(classe)+"/"+file)

            #prds_crop = crop_center(prds_crop,50,50)         
            prds = prds.max(0)
...
def count_classes2 (root):

    index = 0

    files = os.listdir(root)
    tam_dataset = len(files)
    
    X = np.zeros([tam_dataset,10])
    
    for file in files:
        X[index][9] = int(file.split('.')[0])
        
        prds_crop = np.load(root+"/"+file)
        h, w = prds_crop.shape
...
root = '/mnt/DADOS_PARIS_1/matheusp/dengue/chessmix/forward_outputs/fcn-resnet101/probs_campinas_inteira'

X = count_classes2(root)
df = pd.DataFrame(X)

df.to_csv("stats_campinas_inteira.csv")
